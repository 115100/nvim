...
vim.api.nvim_create_autocmd({'FileType'}, {pattern={'ada', 'bzl', 'cabal', 'groovy', 'haskell', 'python' ,'yaml'}, command = 'setlocal expandtab'})
-- fix stupid indentation problems with bazel/yaml
vim.api.nvim_create_autocmd({'FileType'}, {pattern={'bazel'}, command = 'setlocal ts=4 sts=4 sw=4 indentkeys-=<:>'})
vim.api.nvim_create_autocmd({'FileType'}, {pattern={'yaml', 'yml'}, command = 'setlocal ts=2 sts=2 sw=2 indentkeys-=0# indentkeys-=<:>'})
